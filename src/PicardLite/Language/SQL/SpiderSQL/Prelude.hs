module PicardLite.Language.SQL.SpiderSQL.Prelude where
...
isKeyword s =
  caselessString s <* notFollowedBy (try alphaNum <|> try (char '_')) <?> s
...
tableNameP = do
  ...
  choice $ fmap
    (try . p . Text.unpack)
    (sortBy (compare `on` (negate . Text.length))
            (HashMap.elems sqlSchema_tableNames)
    )
...
columnNameP = do
  ...
  choice $ fmap
    (try . p . Text.unpack)
    (sortBy (compare `on` (negate . Text.length))
            (HashMap.elems sqlSchema_columnNames)
    )
...
toSpiderTyp (ColumnType__UNKNOWN x) =
  unexpected $ "unexpected column type " <> show x
...
columnTypeAndNameP
  :: (CharParsing m, MonadReader SQLSchema m) => m (SpiderTyp, String)
columnTypeAndNameP = do
  ...
  choice $ fmap
    ...
    (sortBy
      ...
      (HashMap.elems $ HashMap.intersectionWith (,)
...
quotedString n =
  flip (<?>) "quotedString"
    $   try
          (between (try isSingleQuote)
                   (try isSingleQuote)
                   (try $ manyAtMost n (notChar '\''))
          )
    <|> try
          (between (try isDoubleQuote)
                   (try isDoubleQuote)
                   (try $ manyAtMost n (notChar '"'))
          )
