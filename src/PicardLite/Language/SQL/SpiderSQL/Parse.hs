...
module PicardLite.Language.SQL.SpiderSQL.Parse where
...
import           PicardLite.Language.SQL.SpiderSQL.Prelude
                                                ( columnNameP , columnTypeAndNameP
                                                , doubleP , eitherP , intP
                                                , isAnd , isAs , isAsc
                                                , isAvg , isBetween , isClosedParenthesis
                                                , isComma , isCount , isDesc
                                                , isDistinct , isDivide , isDot
                                                , isEq , isExcept , isFrom
                                                , isGe , isGroupBy , isGt
                                                , isHaving , isIn , isIntersect
                                                , isJoin , isLe , isLike
                                                , isLimit , isLt , isMax 
                                                , isMin , isMinus , isNe
                                                , isNot , isOn , isOpenParenthesis
                                                , isOr , isOrderBy , isPlus
                                                , isSelect , isStar , isSum
                                                , isTimes , isUnion , isWhere
                                                , manyAtMost , quotedString , tableNameP
                                                , toColumnType
                                                )
import           PicardLite.Language.SQL.SpiderSQL.Syntax
                                                ...
                                                , pattern ColumnUD
                                                , Cond(..)
                                                , pattern DistinctColUnitUD
                                                , From(..)
                                                , OrderBy(..)
                                                , OrderByOrder(..)
                                                , SX(..)
                                                , Select(..)
                                                , pattern SelectDistinctUD
                                                , pattern SelectUD
                                                , SpiderSQL(..)
                                                , pattern SpiderSQLUD
                                                , SpiderTyp(..)
...
import qualified PicardLite.Types
import           PicardLite.Types               ( SQLSchema(..) )
...
data ParserState x = ParserState
  ...
  , psTables
      :: HashMap.HashMap (Either TableId (Select x)) (HashSet.HashSet Scope)
  , psCurScope :: Scope
...
mkParserStateUD = ParserState { psAliases  = mempty
                              , psTables   = mempty
                              , psCurScope = minBound
                              , psGuards   = mempty
                              }
...
mkParserStateTC = ParserState { psAliases  = mempty
                              , psTables   = mempty
                              , psCurScope = minBound
                              , psGuards   = mempty
                              }
...
type MonadSQL x m
  = (MonadPlus m, MonadState (ParserState x) m, MonadReader (ParserEnv x) m)
...
betweenParentheses = between (try $ isOpenParenthesis <* spaces)
...
select sx = flip (<?>) "select" $ do
  ...
  aggs     <- sepBy (try $ betweenOptionalParentheses (agg sx))
                    (try $ spaces *> isComma <* spaces)
  case sx of
...
valUnit
  :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m (ValUnit x)
valUnit sx = flip (<?>) "valUnit" $ do
  column      <- val sx
  maybeBinary <- do
    let vTyp :: ValTC -> m SpiderTyp
        ...
        checkedBinary
          :: SX x
          -> (XValUnit x -> Val x -> Val x -> ValUnit x)
          -> Val x
          -> Val x
          -> m (ValUnit x)
...
colUnit
  :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m (ColUnit x)
colUnit sx = flip (<?>) "colUnit" $ do
  ...
  case sx of
    SUD -> do
      modify
        (  field @"psGuards"
        %~ HashMap.insertWith HashSet.union curScope (HashSet.singleton v)
        )
      case distinct of
...
inTable sx SQLSchema {..} ColumnId {..} TableId {..} =
  let matchingColumnUIds :: SX x -> m [PicardLite.Types.ColumnId]
      matchingColumnUIds SUD =
        pure
          . HashMap.keys
          . HashMap.filter
              (\x -> Text.toLower (Text.pack columnName) == Text.toLower x)
      ...
      columnUIdToTableName =
        sqlSchema_tableNames `HashMap.compose` sqlSchema_columnToTable
...
inSelect sx c    s = case s of
 ...
 where
  ...
  go [] = []
  go (Agg _ _ (Column _ (ValColUnit _ ColUnit {..})) : aggs) =
    colUnitColId : go aggs
  go (Agg _ _ (Column _ (ValColUnit _ DistinctColUnit {..})) : aggs) =
    distinctColUnitColdId : go aggs
  go (Agg{} : aggs) = go aggs
...
guardTableColumn
  :: forall x m
   . (Parsing m, MonadState (ParserState x) m)
  => SX x
  -> SQLSchema
  -> TableId
  -> ColumnId x
...
guardTableColumn sx sqlSchema t c = do
  ...
  if memberT psTables
    then do
      cInT <- c `inTable'` t
      if cInT then pure ColumnInTable else pure ColumnNotInTable
  ...
  case HashMap.lookup a psAliases of
    ...
    Just m  -> case Map.lookupLE psCurScope m of
      ...
      Just (_, TableUnitSQL SpiderSQL {..} _) ->
        pure $ if inSelect sx c spiderSQLSelect
          then ColumnInAlias
      ...
      Just (_, Table t _) ->
        ...
        in  do
              cInT <- c `inTable'` t
              if cInT then pure ColumnInAlias else pure ColumnNotInAlias

guardColumn
  :: forall x m
   . (Parsing m, Monad m, MonadState (ParserState x) m)
  => SX x
  -> SQLSchema
  -> ColumnId x
  -> m ()
guardColumn sx sqlSchema = go
 where
  ...
  go c    = do
    ParserState {..}       <- get
    columnInTablesPerScope <- Map.fromListWith (+) . join <$> traverse
      (\case
        (Left t, scopes) -> do
    ...
    columnInAliasesPerScope <- Map.fromListWith (+) . join <$> traverse
      (\(_a, scopes) -> traverse
        (\(scope, tu) -> case tu of
    ...
    let columnPerScope =
          Map.unionWith (+) columnInTablesPerScope columnInAliasesPerScope
    unless (maybe False ((== 1) . snd) $ Map.lookupLE psCurScope columnPerScope)
      .  unexpected
...
withGuards
  :: forall x m p
   . (Parsing m, MonadPlus m, MonadState (ParserState x) m)
  => SX x
  -> SQLSchema
  -> m p
  -> m p
withGuards sx sqlSchema p = do
  ...
  let curGuards =
      ...
      f (GuardTableColumn t c) = do
        ...
        case columnInTable of
          TableNotInScope ->
            unexpected $ "table " <> show t <> " is not in scope."
          ColumnNotInTable ->
            unexpected
              $  "column "
              <> (case sx of
      ...
      f (GuardAliasColumn a c) = do
        ...
        case columnInAlias of
          AliasNotInScope ->
            unexpected $ "alias " <> show a <> " is not in scope."
          ColumnNotInAlias ->
            unexpected
              $  "column "
              <> (case sx of
...
tableId
  :: forall x m
   . (CharParsing m, MonadReader (ParserEnv x) m, MonadPlus m)
  => m TableId
tableId =
  ...
  in  flip (<?>) "columnId" $ do
        ...
        case sx of
          SUD ->
            ...
            in  try (isStar $> Star)
                  <|> try (ColumnIdUD <$> terminate columnNameP')
          STC ->
            let columnTypeAndNameP' = runReaderT columnTypeAndNameP sqlSchema
            in  try (isStar $> Star)
                  <|> try (uncurry ColumnId <$> terminate columnTypeAndNameP')
...
from :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m (From x)
from sx = flip (<?>) "from" $ do
 ...
 where
  ...
  p = (,) <$> tableUnit sx <*> many
    ...
    *> isJoin
    *> someSpace
    *> ((,) <$> tableUnit sx <*> optional
         (try $ someSpace *> isOn *> someSpace *> cond sx)
       )
    )
...
tableUnit
  :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m (TableUnit x)
tableUnit sx =
  flip (<?>) "tableUnit"
    $ let aliasP = someSpace *> isAs *> someSpace *> alias'
          tableUnitSQL =
            flip (<?>) "tableUnitSQL"
              $   TableUnitSQL
              <$> betweenParentheses
                    (get >>= spiderSQL sx . (field @"psCurScope" %~ succ))
              <*> optional (try aliasP)
          table =
            flip (<?>) "table" $ Table <$> tableId <*> optional (try aliasP)
      in  do
            tu <- try tableUnitSQL <|> try table
            case tu of
              TableUnitSQL _ (Just a) -> updateAliases sx a tu
              TableUnitSQL SpiderSQL {..} Nothing ->
                updateTables sx (Right spiderSQLSelect)
    ...
    $ let
        mkCond p' =
          let suffix r' =
              ...
              r = choice [try $ Not <$> (isNot *> spaces *> p'), try p']
          in  suffixRec r
        starEq TStar _     = True
        starEq _     TStar = True
        ...
        checkedEquality STC f vu vu'
          ...
          = unexpected
            ...
            <> " and "
            <> show vu'
            <> " do not work for equality conditions"
        checkedNumerical
          :: SX x
          -> (ValUnit x -> ValUnit x -> Cond x)
          -> ValUnit x
        ...
        checkedNumerical STC f vu vu'
          | valUnitTyp vu `starEq` TNumber && valUnitTyp vu' `starEq` TNumber
          = pure $ f vu vu'
          | valUnitTyp vu `starEq` TTime && valUnitTyp vu' `starEq` TTime
          = pure $ f vu vu'
          | otherwise
          = unexpected
            $  "the types of "
            ...
            <> " and "
            <> show vu'
            <> " do not work for numerical comparison conditions"
        checkedLikeness
          :: SX x
          -> (ValUnit x -> ValUnit x -> Cond x)
          -> ValUnit x
        ...
        checkedLikeness STC f vu vu'
          | valUnitTyp vu `starEq` TText && valUnitTyp vu' `starEq` TText
          = pure $ f vu vu'
          | otherwise
          = unexpected
            $  "the types of "
            ...
            <> " and "
            <> show vu'
            <> " do not work for like conditions"
        checkedBetween
          :: SX x -> ValUnit x -> ValUnit x -> ValUnit x -> m (Cond x)
        ...
        checkedBetween STC vu vu' vu''
          ...
          = unexpected
            ...
            <> ", and "
            <> show vu''
            <> " do not work for between conditions"
        p = choice
        ...
        binary f q = do
          ...
          vu' <- spaces *> q *> spaces *> betweenOptionalParentheses
            (valUnit sx)
          f vu vu'
        binaryNot f q = do
          vu  <- betweenOptionalParentheses (valUnit sx)
          vu' <-
            ...
            *> q
            *> spaces
            *> betweenOptionalParentheses (valUnit sx)
          Not <$> f vu vu'
        between' = do
          vu  <- betweenOptionalParentheses (valUnit sx)
          vu' <-
            someSpace *> isBetween *> someSpace *> betweenOptionalParentheses
              (valUnit sx)
          vu'' <- someSpace *> isAnd *> someSpace *> betweenOptionalParentheses
            (valUnit sx)
...
-- Right (ValSQL {sqlValueX = (), sqlValue = SpiderSQL {spiderSQLX = (), spiderSQLSelect = Select () [Agg () Nothing (Column () (ValColUnit {columnX = (), columnValue = ColUnit {colUnitX = (), colUnitAggId = Nothing, colUnitTable = Nothing, colUnitColId = ColumnId {columnIdX = (), columnName = "Song_ID"}}}))], spiderSQLFrom = From {fromTableUnits = [Table (TableId {tableName = "song"}) Nothing], fromCond = Nothing}, spiderSQLWhere = Nothing, spiderSQLGroupBy = [], spiderSQLOrderBy = Nothing, spiderSQLHaving = Nothing, spiderSQLLimit = Nothing, spiderSQLIntersect = Nothing, spiderSQLExcept = Nothing, spiderSQLUnion = Nothing}})
val :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m (Val x)
...
groupBy
  :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m [ColUnit x]
groupBy sx = flip (<?>) "groupBy" $ isGroupBy *> someSpace *> sepBy1
  (try $ betweenOptionalParentheses (colUnit sx))
  (try $ spaces *> isComma <* someSpace)
...
orderBy
  :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m (OrderBy x)
orderBy sx = flip (<?>) "orderBy" $ do
  ...
  valUnits <-
    let order =
          optional
              (try $ spaces *> (try (isAsc $> Asc) <|> try (isDesc $> Desc)))
            >>= maybe (pure Asc) pure
...
havingCond
  :: forall x m . (TokenParsing m, MonadSQL x m) => SX x -> m (Cond x)
...
spiderSQL
  :: forall x m
   . (TokenParsing m, MonadPlus m, MonadReader (ParserEnv x) m)
  => SX x
  -> ParserState x
  -> m (SpiderSQL x)
spiderSQL sx env = flip (<?>) "spiderSQL" $ flip evalStateT env $ do
  ParserEnv (ParserEnvWithGuards peWithGuards) sqlSchema <- ask
  sel <- select sx
  fro <- peWithGuards sqlSchema $ fromMaybe (From [] Nothing) <$> optional
    (try $ spaces *> from sx)
  whe <- optional (try . peWithGuards sqlSchema $ someSpace *> whereCond sx)
  grp <- fromMaybe []
    <$> optional (try . peWithGuards sqlSchema $ someSpace *> groupBy sx)
  (ord, hav) <- permute
    (    (,)
    <$$> try
           (optional (try . peWithGuards sqlSchema $ someSpace *> orderBy sx))
    <||> try
           (optional
             (try . peWithGuards sqlSchema $ someSpace *> havingCond sx)
           )
    )
  ...
  (int, exc, uni) <- permute
    ...
    <$$> try
           (optional
             (try $ someSpace *> isIntersect *> someSpace *> spiderSQL sx env)
           )
    <||> try
           (optional
             (try $ someSpace *> isExcept *> someSpace *> spiderSQL sx env)
           )
    <||> try
           (optional
             (try $ someSpace *> isUnion *> someSpace *> spiderSQL sx env)
           )
    )
  case sx of
